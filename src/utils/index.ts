...
export function setAppTitle(title?: string) {
  ...
  document.title = title ? title : currentRoute.value.meta.title;
}

// 图标尺寸转换
export function setSizeWithRem(size: number) {
  if (!size) return 14;
  let clientWidth = document.documentElement.clientWidth;
  let deviceWidth = clientWidth > 1024 ? 1920 : 375;

  return ((clientWidth / deviceWidth) * size).toFixed(0);
}
...
export function formatMoney(
  ...
  options: {
    ...
    separator?: string; // 千分位分隔符，默认逗号
    decimal?: string; // 小数点符号，默认点号
    prefix?: string; // 前缀，如￥
    suffix?: string; // 后缀，如元
    showZero?: boolean; // 是否显示零值，默认true
  } = {}
) {
  const { precision = 2, separator = ',', decimal = '.', prefix = '', suffix = '', showZero = true } = options;

  // 处理空值或非数字
  if (value === null || value === undefined || value === '') {
    return showZero ? `${prefix}0${decimal}00${suffix}` : '';
  }
  ...
  // 组合结果
  const result = decimalPart ? `${formattedInteger}${decimal}${decimalPart}` : formattedInteger;
 ...
 * 整数金额格式化（无小数）
 * @param value 金额数值
 * @returns 格式化后的整数金额字符串
 */
export function formatIntegerMoney(value: number | string) {
  return formatMoney(value, { precision: 0 });
}
 ...
 * @returns 格式化后的百分比字符串
 */
export function formatPercent(value: number | string, precision: number = 2) {
  if (value === null || value === undefined || value === '') {
    return '0%';
  }
