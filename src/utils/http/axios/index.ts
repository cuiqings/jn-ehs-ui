...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    // 这里逻辑可以根据项目进行修改
    const hasSuccess = data && isObject(data) && Reflect.has(data, 'code') && (code === ResultEnum.SUCCESS || code === 200);
    if (hasSuccess) {
      if (success && message && options.successMessageMode === 'success') {
        console.log('信息成功提示 options', options,message,globSetting, globSetting.apiUrl);
        //信息成功提示
        if (!message.includes('onlinePreview?url')&&!message.includes('/sys/noAuth/tokenLogin')&&!message.includes('token登录成功')) {
