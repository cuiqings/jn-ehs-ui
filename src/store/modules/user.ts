...
import { ROLES_KEY, TOKEN_KEY, USER_INFO_KEY, LOGIN_INFO_KEY, DB_DICT_DATA_KEY, TENANT_ID, AI_TOKEN_KEY } from '/@/enums/cacheEnum';
...
export const useUserStore = defineStore({
  ...
  getters: {
    ...
    getAiToken(): string {
      return this.aiToken || getAuthCache<string>(AI_TOKEN_KEY);
    },
    getAllDictItems(): [] {
  ...
  actions: {
    ...
    setAiToken(info: string | undefined) {
      this.token = info ? info : ''; // for null or undefined value
      setAuthCache(AI_TOKEN_KEY, info);
    },
      ...
      await loginApi(loginParams, mode)
        .then((res: any) => {
          if (res.success && res.code === 200) {
            ...
            return this.afterLoginAction(goHome, res.result);
          } else {
            notification.error({
              message: t('sys.api.errorTip'),
              description: res.message || t('sys.login.networkExceptionMsg'),
              duration: 3,
            });
    ...
    async logout(goLogin = false) {
      ...
      if (this.getToken) {
        try {
          await doLogout().then((res) => {
            if (res.success) {
              const $ls = createLocalStorage();
              $ls.remove('aiSearchHistory');
              $ls.remove('aiSearchId');
              message.success(t('sys.login.signOut'));
            }
          });
        } catch {
