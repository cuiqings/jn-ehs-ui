      ...
      switch (permissionMode) {
        ...
        case PermissionModeEnum.BACK:
          ...
          try {
            ...
            // if (userStore.getUserInfo.orgCode.startsWith('A04B12')) {
            //   routeList = routeList.map((item) => {
            //     if (item.name === 'dashboard-analysis') item.component = 'sm/home.vue';
            //     return item;
            //   });
            // } else {
            //   routeList = routeList.map((item) => {
            //     if (item.name === 'dashboard-analysis') item.component = 'dashboard/Analysis/index.vue';
            //     return item;
            //   });
            // }
            routeList = routeList.map((item) => {
              if (item.name === 'dashboard-analysis') item.component = 'dashboard/home/index.vue';
              return item;
            });
          ...
          console.log('组件地址前加斜杠处理耗时：', +new Date() - time);
          // 动态引入组件
          time = +new Date();
          if(!isMobile){
            routeList = transformObjToRoute(routeList);
          }
          console.log('动态引入组件：', +new Date() - time);
          // 构建后台路由菜单
          time = +new Date();
          const backMenuList = transformRouteToMenu(routeList);
          console.log('构建后台路由菜单：', +new Date() - time);
          time = +new Date();
          this.setBackMenuList(backMenuList);
          console.log('setBackMenuList：', +new Date() - time);
          // 删除meta.ignoreRoute项
          time = +new Date();
          routeList = filter(routeList, routeRemoveIgnoreFilter);
          console.log('删除meta.ignoreRoute项：', +new Date() - time);
          time = +new Date();
          routeList = routeList.filter(routeRemoveIgnoreFilter);
          console.log('routeList.filter：', +new Date() - time);
          time = +new Date();
          routeList = flatMultiLevelRoutes(routeList);
          console.log('flatMultiLevelRoutes：', +new Date() - time);
          time = +new Date();
