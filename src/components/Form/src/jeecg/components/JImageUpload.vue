<template>
  <div class="clearfix">
    ...
    >
      <template v-if="!showRemoveIcon" #removeIcon><span class="reomve-icon">x</span></template>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // 获取工号
    const userInfo = <any>getLoginBackInfo() || { userInfo: useUserStore().getUserInfo };
    ...
    function beforeUpload(file) {
      ...
      if (props.isWatermark || props.isWXWatermark) {
        return new Promise((resolve) => {
          ...
          reader.onload = (e: any) => {
            ...
            img.onload = () => {
              ...
              let data = '';
              let watermarkText = '';
              if (props.isWXWatermark)
                watermarkText = `${dateFormat(new Date(), 'yyyy-MM-dd')} ${userInfo.userInfo.workNo} ${userInfo.userInfo.realname} 上传`;
              else watermarkText = `${dateFormat(new Date(), 'yyyy-MM-dd')} ${userInfo.userInfo.workNo} 上传`;
              const imgWidth = img.width;
              const imgHeight = img.height;
              canvas.width = imgWidth; // 画布宽度
              canvas.height = imgHeight; // 画布高度
              ctx.drawImage(img, 0, 0, imgWidth, imgHeight); // 绘制图片大小和先前图片一致
              const fontSize = imgWidth / 39; // 根据图片大小确定文字大小
              for (let i = 0; i < 5; i++) {
                ctx.save();
                ctx.translate(i + imgWidth / 20, i + 50);
                ctx.rotate((45 * Math.PI) / 300);
                ctx.fillStyle = 'rgb(0,0,0,0.2)'; // 水印颜色，透明度
                ctx.textBaseline = 'center'; // 水印对其的基准线
                ctx.font = `${fontSize}px Verdana`; // 文字大小
                ctx.fillText(watermarkText, (imgWidth / 2.5) * i, fontSize * 0.4); // 添加的文字
                ctx.restore();
                for (let j = 1; j <= 18; j++) {
                  ctx.save();
                  ctx.translate(i + imgWidth / 20, i + 50);
                  ...
                  ctx.textBaseline = 'center'; // 水印对其的基准线
                  ctx.font = `${fontSize}px Verdana`; // 文字大小
                  ctx.fillText(watermarkText, (imgWidth / 2.5) * i, fontSize * j * 4); // 添加的文字
                  ctx.restore();
                }
                for (let j = 1; j <= 10; j++) {
                  ctx.save();
      ...
      if (props.isYhWatermark || props.isP_jType) {
        ...
        // if (!isLt300M) {
        //   createMessage.error('图片过大，请不要超过10M！');
        //   return false || Upload.LIST_IGNORE;
        // }
      }
      if (props.isYhWatermark || props.isFireWatermark) {
        return new Promise((resolve) => {
          const reader = new FileReader();
          reader.readAsDataURL(file); // file转base64
          reader.onload = (e: any) => {
            ...
            img.onload = () => {
              ...
              let data = '';
              let watermarkText = '';
              if (props.isYhWatermark)
                watermarkText = `${userInfo.userInfo.realname} ${userInfo.userInfo.workNo} ${dateFormat(new Date(), 'yyyy-MM-dd hh:mm:ss')} 上传`;
              if (props.isFireWatermark)
                watermarkText = `${userInfo.userInfo.realname} ${userInfo.userInfo.workNo}上传于${dateFormat(new Date(), 'yyyy-MM-dd hh:mm:ss')}`;
              const imgWidth = img.width;
              const imgHeight = img.height;
              canvas.width = imgWidth; // 画布宽度
              canvas.height = imgHeight; // 画布高度
              const fontSize = imgWidth / 35; // 根据图片大小确定文字大小
              ctx.drawImage(img, 0, 0, imgWidth, imgHeight); // 绘制图片大小和先前图片一致
              ctx.fillStyle = '#fff';
              ...
              ctx.shadowColor = '#000';
              ctx.shadowBlur = '2';
              ctx.fillText(watermarkText, 20, 30);
              data = canvas.toDataURL(file.type); // 输出压缩后的base64
              // base64转file
              const arr: any = data.split(',');
              const mime = arr[0].match(/:(.*?);/)[1];
    ...
    function handleChange({ file, fileList }) {
      // initTag.value = false;
      // 过滤 beforeUpload return false 的文件
      if ((!file.status || ['error'].includes(file.status)) && fileList.length > 0) {
        fileList = fileList.filter((item) => !!item.status && !['error'].includes(item.status));
      }
      if (file.status === 'done' && file.response.code != 200) {
        createMessage.error(`${file.response.message}`);
        fileList = fileList.filter((item) => !(item.status === 'done' && item.response.code != 200));
      }
