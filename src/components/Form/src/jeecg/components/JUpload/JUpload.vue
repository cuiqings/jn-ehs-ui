<template>
  <div ref="containerRef" :class="`${prefixCls}-container`">
    <a-upload
      :headers="headers"
      :multiple="multiple"
      :action="isFileManage ? fileUploadFile : uploadUrl"
      :fileList="fileList"
      :disabled="disabled"
      v-bind="bindProps"
      @remove="onRemove"
    ...
    >
      ...
      <a-button :type="buttonType" v-else-if="buttonVisible" :disabled="isMaxCount || disabled">
...
<script lang="ts" setup>
  ...
  import { previewFile, doPreview, uploadUrl, fileUploadFile } from '/@/api/common/api';
  ...
  import { getFileAccessHttpUrl, getFileManageHttpUrl } from '/@/utils/common/compUtils';
  ...
  const props = defineProps({
    ...
    isFileManage: propTypes.bool.def(false),
    buttonType: propTypes.string.def('default'),
  });
  ...
  function parsePathsValue(paths) {
    ...
    for (const item of paths.split(',')) {
      let url = props.isFileManage ? getFileManageHttpUrl(item) : getFileAccessHttpUrl(item);
      list.push({
        uid: uidGenerator(),
  ...
  function parseArrayValue(array) {
    ...
    for (const item of array) {
      let url = props.isFileManage ? getFileManageHttpUrl(item.filePath) : getFileAccessHttpUrl(item.filePath);
      list.push({
        uid: uidGenerator(),
        name: item.fileName,
        url: url,
      ...
      if (info.file.response.success) {
        successFileList = fileListTemp.map((file) => {
          if (file.response) {
            ...
            file.url = props.isFileManage ? getFileManageHttpUrl(reUrl) : getFileAccessHttpUrl(reUrl);
          }
          return file;
        });
  ...
  function onFilePreview(file) {
    if (isImageMode.value) {
      ...
      if (props.isFileManage) {
        doPreview(file?.response?.message).then((res) => {
