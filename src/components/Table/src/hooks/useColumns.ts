...
function handleItem(item: BasicColumn, ellipsis: boolean, resizable: boolean) {
  const { key, dataIndex, children } = item;
  item.align = item.align || DEFAULT_ALIGN;
  if (ellipsis || resizable) {
    if (!key) {
      item.key = dataIndex;
    }
  ...
  if (children && children.length) {
    handleChildren(children, !!ellipsis, !!resizable);
  }
}

function handleChildren(children: BasicColumn[] | undefined, ellipsis: boolean, resizable: boolean) {
  if (!children) return;
  children.forEach((item) => {
    const { children } = item;
    handleItem(item, ellipsis, resizable);
    handleChildren(children, ellipsis, resizable);
  });
}
...
export function useColumns(
  ...
  const getColumnsRef = computed(() => {
    ...
    columns.forEach((item) => {
      ...
      handleItem(
        ...
        Reflect.has(item, 'resizable') ? !!item.resizable : !!resizable
      );
    });
    return columns;
  });
  ...
  // 更新Columns
  function setReplaceColumns(dataIndex: string | undefined, value: Partial<BasicColumn>) {
    if (!dataIndex || !value) {
      return;
    }
    const arr = columnsRef.value.map((item) => {
      if (item.dataIndex === dataIndex) {
        return value;
      } else {
        if (item.children) {
          setCacheColumns(dataIndex, value, item.children);
        }
        return item;
      }
    });
  ...
  function setCacheColumns(dataIndex, value, columns: BasicColumn[]) {
    columns.forEach((item) => {
      if (item.dataIndex === dataIndex) {
        return value;
      } else {
