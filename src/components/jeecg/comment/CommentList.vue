...
export default defineComponent({
  ...
  props: {
    ...
    allHeight: propTypes.number.def(400),
    commentHeight: propTypes.number.def(300),
  },
  setup(props) {
    ...
    function getMyname() {
      if (userInfo.realname) {
        return userInfo.realname.substr(0, 1);
      }
      return '';
    }
    ...
    function getAvatarText(item) {
      if (item.fromUserId_dictText) {
        return item.fromUserId_dictText.substr(0, 1);
      }
      return '未知';
    }
    ...
    const commentHeight = ref(props.commentHeight);
    const allHeight = ref(props.allHeight);
    onMounted(() => {
