<template>
  <div v-if="visible">
    <a-alert type="info" class="jeecg-comment-files" style="margin: 0">
      <template #message>
        <span class="j-icon">
          <a-upload multiple v-model:file-list="selectFileList" :showUploadList="false" :before-upload="beforeUpload" :max-count="10">
          ...
          <div class="complex">
            ...
            <div class="layer" :class="{ 'layer-image': isImage(item) }">
              <div class="next" @click="viewImage(item)">
                ...
                <Tooltip title="预览">
                  <eye-outlined :style="{fontSize: '16px', color: '#1890ff'}" />
                </Tooltip>
              </div>
              <div class="buttons">
                <div>
                  <Tooltip title="删除">
                    <delete-outlined :style="{fontSize: '16px', color: '#1890ff'}" @click="handleRemove(item)" />
                  </Tooltip>
                </div>
              </div>
            </div>
...
import { EyeOutlined, UploadOutlined, FolderOutlined, DownloadOutlined, PaperClipOutlined, DeleteOutlined } from '@ant-design/icons-vue';
export default {
  name: 'UploadChunk',
  components: {
    Tooltip,
