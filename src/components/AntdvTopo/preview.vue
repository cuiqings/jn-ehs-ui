<template>
  <div class="antv-wrapper" @click="wrapClick">
    <div class="wrapper-canvas" id="wrapper"></div>
    <div
      class="node-apv"
      v-if="showApv && showDialog"
      :style="{
        left: curData.x + 10 + 'px',
        top: curData.y + 10 + 'px',
      }"
      @click.stop="cellClick"
    >
      <div class="n-title">
        <span>详情</span>
        <a-icon type="close" @click="closed" />
      </div>
      <dl class="n-list">
        <dt>设备名称：</dt>
        <dd>{{ curData.deviceName || '--' }}</dd>
      </dl>
      <dl class="n-list">
        <dt>设备编号：</dt>
        <dd>{{ curData.deviceCode || '--' }}</dd>
      </dl>
      <dl class="n-list">
        <dt>{{ curData.name || '--' }}：</dt>
        <dd>{{ (curData.value || '--') + (curData.unit || '--') }}</dd>
      </dl>
      <div class="n-go">
        <span @click="goDetail(curData.deviceId)">查看历史</span>
      </div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    beforeUnmount() {
      // const dom = document.querySelectorAll('.x6-cell')
      // for(let i=0;i<dom.length;i++){
      //   dom[i].removeEventListener("click", this.cellClick)
      // }
    },
    methods: {
      // 添加cell点击监听
      addCellListener() {
        ...
        const dom = document.querySelectorAll('.x6-cell');
        for (let i = 0; i < dom.length; i++) {
          dom[i].addEventListener('click', this.cellClick);
        }
      },
      // 初始化渲染画布
      initGraph(obj) {
