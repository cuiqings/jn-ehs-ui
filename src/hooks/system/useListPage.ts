import { reactive, ref, Ref, unref, createVNode } from 'vue';
...
interface OperateRequestOptions {
  // 是否显示确认对话框，默认 true
  confirm?: boolean;
  // 是否自动刷新表格，默认 true
  reload?: boolean;
...
export function useListPage(options: ListPageOptions) {
  ...
  // 导出 excel
  async function onExportXls(newParams) {
    // 查询参数请求前处理
    //update-begin---author:wangshuai ---date:20220411  for：导出新增自定义参数------------
    const { url, name, params, suffix = 'xls' } = options?.exportConfig ?? {};
    ...
    if (realUrl) {
      ...
      let paramsForm: any = {};
      if (options.tableProps.formConfig) {
        // 查询参数请求前处理（区间处理）
        if (options.tableProps.beforeFetch) paramsForm = await options.tableProps?.beforeFetch(await getForm().validate());
        else paramsForm = await getForm().validate();
      }
      //update-begin-author:liusq date:20230410 for:[/issues/409]导出功能没有按排序结果导出,设置导出默认排序，创建时间倒序
      if (!paramsForm?.column) {
        Object.assign(paramsForm ? paramsForm : {}, { column: 'createTime', order: 'desc' });
      }
      //update-begin-author:liusq date:20230410 for: [/issues/409]导出功能没有按排序结果导出,设置导出默认排序，创建时间倒序

      // hypo add 20231127
      if (newParams) {
        Object.assign(paramsForm ? paramsForm : {}, newParams);
      }
      ...
      if (suffix === 'xls') return handleExportXls(title as string, realUrl, filterObj(paramsForm), selectedRowKeys);
      if (suffix === 'xlsx') return handleExportXlsx(title as string, realUrl, filterObj(paramsForm), selectedRowKeys);
  ...
  function onImportXls(file) {
    const { url, success, data } = options?.importConfig ?? {};
    //update-begin-author:taoyan date:20220507 for: erp代码生成 子表 导入地址是动态的
    const realUrl = typeof url === 'function' ? url() : url;
    if (realUrl) {
      const dataBody = typeof data === 'function' ? data() : data;
      return handleImportXls(file, realUrl, success || reload, dataBody);
      //update-end-author:taoyan date:20220507 for: erp代码生成 子表 导入地址是动态的
    } else {
  ...
  function operateRequest(api: () => Promise<any>, options: OperateRequestOptions, callback?: Function) {
    return new Promise((resolve, reject) => {
      const execute = async () => {
        try {
          setLoading(true);
      ...
      if (options?.confirm) {
        $message.createConfirm({
          ...
          title: options?.title ? options.title : options?.isBatch ? '批量删除' : '删除',
          content: options?.content ? options.content : options?.isBatch ? `是否删除已选中的${selectedRowKeys.value.length}项？` : '确定要删除吗？',
          onOk: () => execute(),
        });
      } else {
        execute();
