    ...
    <div>
      ...
      >
        <div class="list" v-for="item in listArr" :key="item.id" @click="handleView(item)">
          ...
          <div> 状态：{{ stausText(item.status) }} </div>
        </div>
      </van-list>
...
<script setup name="app-projectManage-list" lang="ts">
  import { list } from '../../projectManage/url/index';
  import { getDepart3ListWithSecurity } from '../../admissionApplication/url/index';
  ...
  const reset = () => {
    ...
    queryForm.territorialUnit = undefined;
    queryForm.status = '';
  };
  const handleConfirm = () => {
    queryForm.pageNo = 1;
  ...
  const stausText = (type) => {
    const text = statusOptions.filter((item) => item.value === type)[0].label;
    return text;
  };
