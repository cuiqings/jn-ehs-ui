...
<script lang="ts" setup>
  ...
  import { examPageList, examConfig } from '../../index.api';
  ...
  const activeIdx = computed(() => appStore.getActiveIdx - 1);
  ...
  const tabChange = (e) => {
    if (e == '0' && list.value.length == 0) {
      onRefresh({});
    }
    if (e == '1' && list2.value.length == 0) {
      onRefresh({});
    }
    if (e == '2' && list3.value.length == 0) {
      onRefresh({});
    }
    if (e == '3' && list4.value.length == 0) {
      onRefresh({});
    }
  };
