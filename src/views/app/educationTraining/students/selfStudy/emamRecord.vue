    ...
    <div class="record">
      ...
      <div class="main">
        <div class="record-item" v-for="item in recordList">
          <div class="left">
            <div class="date">{{ item.paperName }}</div>
...
<script lang="ts" setup>
  import { getExamInfo } from '/@/views/education/student/selfStudy/url/index';
  ...
  pageInit();
  function pageInit() {
    getExamInfo(props.practiseId).then((res) => {
      console.log(res);
  ...
  const go = (item) => {
    ...
    sessionStorage.setItem('examResult', JSON.stringify(item));
    router.push('/app/student/exam?status=1&taskId=' + props.practiseId);
  };
</script>
