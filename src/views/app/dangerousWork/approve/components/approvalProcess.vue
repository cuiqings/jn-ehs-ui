    ...
    <van-form ref="formRef">
      <van-steps direction="vertical" :active="0">
        <template v-for="(item, idx) in stepArrs" :key="idx">
          <van-step>
            <div :class="{'alreadyexam': item.state == 0}">
              <h3>{{ item.nodeName }}</h3>
            ...
            <template #active-icon>
              <span :class="['step-icon', item.state == 0 ? 'step-icon-active' : '']">{{ idx + 1 }}</span>
            </template>
...
<script lang="ts" setup>
  import { getApplyForUnit, getApproval, getWorkJobWorkDetail } from '../../../../hazardousOperation/api/index';
