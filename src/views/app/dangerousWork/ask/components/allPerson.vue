          ...
          <div class="par-left">
            <ul>
              <VirtualScroll height="291" :items="userList" item-height="35" v-slot="{ item }">
                <li @click="onBackName(item)" :style="{ backgroundColor: selectIds.includes(item.id) ? '#bae7ff' : '' }"> {{ item.name }} </li>
              </VirtualScroll>
            </ul>
          </div>
          <div class="par-right">
  ...
  async function comUser(code?: string | null, name?: string) {
    ...
    await userListByOrg({ ...params, pageNo: 1, pageSize: 100000 }).then((res) => {
      res.records.forEach((item) => {
        item.name = `${item.realname}-${item.workNo}-${item.orgSx}`;
      });
