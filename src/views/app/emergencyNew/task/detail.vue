          ...
          <van-field required label="4、演练记录">
            <template #input>
              <div class="record-list">
                <div class="tit"> 动员培训<span v-if="detailInfo.drillRecord" @click="getHtml(1)">动员培训.pdf</span> </div>
                <div class="tit"> 演练记录<span v-if="detailInfo.drillRecord" @click="getHtml(2)">演练记录.pdf</span> </div>
                <div class="tit"> 评估报告<span v-if="detailInfo.drillRecord" @click="getHtml(3)">评估报告.pdf</span> </div>
                <!-- <div class="tit"> 应急救援队伍名单<span v-if="detailInfo.drillRecord" @click="getHtml(4)">应急救援队伍名单.pdf</span>
                </div> -->
                <div class="tit"> 演练评价表<span v-if="detailInfo.drillRecord" @click="getHtml(5)">演练评价表.pdf</span> </div>
              </div>
            </template>
          </van-field>
      ...
      <van-collapse-item title="计划审批" name="3">
        <ul>
          <li v-for="item in detailInfo.examineList">
            ...
            <div class="name" v-if="item.backReason">退回原因：<span style="color: #ee0a24;">{{ item.backReason }}</span></div>
...
import { taskDetail, approval, drillTaskView } from '/@/views/emergencyNew/api';
