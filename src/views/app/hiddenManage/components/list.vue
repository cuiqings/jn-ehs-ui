...
<script lang="ts" setup>
  ...
  import ListItem5 from './listItem5.vue';
  import ListItem6 from './listItem6.vue';
  import ListItem7 from './listItem7.vue';
  ...
  /**
   * 编辑事件：将当前项的编辑意图上抛给父组件
   * 父组件（index.vue）据此跳转到 add.vue 并携带 id
   */
  const editItem = (item) => {
    emits('editItem', item);
  };
  ...
  /**
   * 删除事件：将当前项的删除意图上抛给父组件
   * 父组件（index.vue）据此弹出确认并调用删除接口
   */
