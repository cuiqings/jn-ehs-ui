  ...
  <div class="item" @click="openDetail">
    ...
    <div class="right">
      ...
      <div class="btn">
        ...
        <span :class="['status', 'tag' + curItem.yhStatus]" v-if="!needBtn">{{
          { 1: '确认退回', 2: '待确认', 3: '待整改', 4: '待验收', 5: '验收通过', 6: '超期未整改' }[curItem.yhStatus]
        }}</span>
        <van-button type="primary" v-if="curItem.canHandle && curItem.suspendFlag !== '1' && needBtn" @click.stop="edit">{{ btnText }}</van-button>
        <!-- <van-button type="primary" @click.stop="edit">{{ btnText }}</van-button> -->
      </div>
    </div>
  </div>
</template>
