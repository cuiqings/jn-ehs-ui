...
<script lang="ts" setup>
  import { jnYhConfirmList, jnYhRepairLogList, jnYhConfirmCheckList, statisticsList, changeRepairPerson, getAllUserByOrg } from '../index.api';
...
const getPopupContainer = () => document.body;
  ...
  const edit = (item) => {
    ...
    appStore.setActiveIdx(active.value);
    let query = { id: item.id, sourceType: item.sourceType, pageType: tabIdx };
  ...
  const goDetail = (item) => {
    appStore.setActiveIdx(active.value);
    let query = { id: item.id, sourceType: item.sourceType, canHandle: false };
    router.push({
