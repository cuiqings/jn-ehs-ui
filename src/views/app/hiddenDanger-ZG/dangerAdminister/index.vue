...
<script lang="ts" setup>
  ...
  const onRefresh = async (params) => {
    if (active.value == '1') {
      ...
      const res = await getList4({...params, ...queryParams.value}).catch(() => listRef4.value.refreshEnd());
  ...
  const getOrgList = () => {
    getOrgCodeList().then((res) => {
      ...
      if(res && res.length == 1){
        ...
        queryParams.value.org = res[0].org;
  ...
  const edit = (item, tabIdx) => {
    ...
    if (tabIdx < 3) {
      ...
      if(item.currentNode == '11'){
        // path = '/app/hiddenDangerNew/dangerAdminister/add';
      }
    }
