...
<template>
  <div class="item">
    ...
    <!-- <div class="bt"><span>执行时间：{{ curItem.updateTime }}</span><span class="blue">{{ curItem.checkCycle_dictText }}</span></div> -->
    <div>所属单位：{{ curItem.orgName }}>{{ curItem.departName }}</div>
    <div class="bt"><span>任务时间:{{ curItem.realCheckTime }}</span></div>
  </div>
</template>
