<template>
  <div class="report-forms">
    ...
    <van-calendar :allow-same-day="true" :min-date="minDate" v-model:show="calendarShow" type="range" @confirm="onConfirm($event, 'calendar')" />
    ...
    <template v-if="route.name == 'AppDangerReportForms'">
      <div class="center">
...
<script lang="ts" setup>
  ...
  watch(route, (n) => {
    if (n.name == 'AppDangerReportForms') {
      pageInit();
    }
  });
  console.log('userInfo.value.orgCode-----', userInfo.value.orgCode);
  ...
  if (userInfo.value.orgCode.length > 6) {
    queryParams.org = userInfo.value.orgCode.slice('0', '6');
    queryParams.depart = userInfo.value.orgCode.substr(0, 9);
  } else {
    queryParams.org = userInfo.value.orgCode;
  ...
  const reset = () => {
    ...
    orgTree.value.map((val) => {
        if (val.orgCode == queryParams.org) {
          queryParams.org = val.orgCode;
          queryParams.orgName = val.departName;
        }
      });
  ...
  const searchClick = () => {
    show.value = false;
    pageInit();
    if (route.name === 'AppDangerReportFormsPagetwo') {
      listPageRef.value?.onRefresh(queryParams);
    }
  };

  const pageInit = () => {
    loading.value = true;
    Promise.all([getDepart2List({}), statistics1(queryParams), statistics2(queryParams), statistics3(queryParams)]).then((res) => {
      loading.value = false;
      orgTree.value = res[0];
      if (orgTree.value.length == 1) {
        queryParams.org = orgTree.value[0].orgCode;
  ...
  function renderComplete(data: any) {
    ...
    xAxisData.map((v) => {
      data1.push(data[v][1]);
      data2.push(data[v][2]);
      data3.push(data[v][3] && data[v][3] > 0 ? (data[v][3] * 100).toFixed(0) : 0);
    });
  ...
  // 跳转到列表页
  const pieClick = () => {
    if (route.name == 'AppDangerReportFormsPagetwo') return;
    router.push({
  ...
  const onConfirm = (value: any, type) => {
    if (type == 'calendar') {
      ...
      queryParams.beginTime = dayjs(value[0]).format('YYYY-MM-DD');
      queryParams.endTime = dayjs(value[1]).format('YYYY-MM-DD');
