...
<template>
  <div class="item" @click="openDetail">
    ...
    <div class="right">
      <p>
        {{ curItem.yhPlaceOrgName }} <template v-if="curItem.yhPlaceOrgName && curItem.yhPlaceDepartName">></template> {{ curItem.yhPlaceDepartName }}
        <span class="tag gy" v-if="item.handleResult == 1">非隐患</span>
        <span class="tag bl" v-if="item.yhLevel == 1">一般</span>
        <span class="tag or" v-if="item.yhLevel == 2">较大</span>
        <span class="tag" v-if="item.yhLevel == 3">重大</span>
      </p>
      <div class="content">
        {{ curItem.yhDescription }}
      </div>
      <div v-if="curItem.reportType == 2">
        <span class="top-f">上级发现</span>
      </div>
      <div v-if="curItem.reportType == 1">
        <span class="top-o">单位自查</span>
      </div>
      <div class="btn">
        ...
        <van-button type="primary" v-if="needBtn && curItem[props.btnShowTag]" @click.stop="edit">{{ btnText }}</van-button>
      </div>
