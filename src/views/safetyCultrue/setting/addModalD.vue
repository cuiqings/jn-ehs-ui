<template>
  <BasicModal @register="registerModal" v-bind="$attrs" :title="title" @ok="submit" @close="close" @cancel="close">
    <a-form ref="formRef" :model="formState" layout="vertical">
      <a-form-item label="安全提示内容" name="playContent" :rules="[{ required: true, message: '请输入' }]">
        <a-textarea :auto-size="{ minRows: 2, maxRows: 5 }" v-model:value="formState.playContent" :maxLength="80" placeholder="请输入安全提示" />
      </a-form-item>
...
<script lang="ts" setup>
  ...
  const title = ref('新增安全提示');
  const [registerModal, { closeModal }] = useModalInner((data) => {
    title.value = data.title;
    if (title.value == '编辑安全提示') {
