<template>
  <div>
    <BasicModal v-bind="$attrs" @register="registerModal" title="选择审核人" @ok="onSubmit" width="900px">
      ...
      <a-row justify="space-between">
            ...
            <a-row>
              ...
              <a-col :span="12" style="padding-left: 10px;">
                <a-spin :spinning="spinning">
                  ...
                  <div class="ur-ovl" v-if="plainOptions.length >0">
        ...
        <a-col flex="300px" class="rp-col">
          <div class="r-title" v-if="selectValues.length >0">
            已选{{selectValues.length}}项
            <span @click="clearSelect">清空</span>
          </div>
          <div class="r-title" v-else>
            暂无数据
          </div> 
          <div class="ur-ovl">
...
<script lang="ts" setup>
  ...
  // 获取人员
  function getUserInfoList(val?: string) {
    const params={
      realname: val
    }
    defHttp.post({ url: '/jn/common/getUserInfoList', params }).then((res) => {
      if(res != null && res.length >0){

          plainOptions.value = res.map((item:any) => {
          return {
            name: item.realname,
            value: item.id,
            orgCode: item.orgCode
