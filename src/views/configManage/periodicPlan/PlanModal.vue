<template>
  <div>
    <BasicModal v-bind="$attrs" @register="registerModal" title="修改审批角色/人" @ok="onSubmit" width="900px">
...
const emit = defineEmits(['success', 'register']);
  ...
  async function onSubmit() {
    ...
    const params = {
      ...
      approvalContentOne: selectValues.value.map(v => v.value).join(','),
      approvalContentTwo: selectValues.value.map(v => v.name).join(',')
    }
