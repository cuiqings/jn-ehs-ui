...
export const useContent = (
  ...
  const init = () => {
    ...
    if (queryParams.time && queryParams.time.length > 0) {
      params.startTime = queryParams.time[0];
      params.endTime = queryParams.time[1];
    } else {
      params.startTime = '';
      params.endTime = '';
    }
    delete params.time;
    initAccidentTypeCharts(params);
    initTrendOccursCharts(params);
    initEventResultCharts(params);
    initOccurUnit(params);
    initLostDaysCharts(params);
    initLostDaysSectorCharts(params);
  };
  onMounted(async () => {
  ...
  const initAccidentTypeCharts = async (params) => {
    ...
    const option = {
      ...
      legend: {
        ...
        top: 40, // 图例距离容器上边缘的距离为20px
        left: 10, // 图例距离容器左边缘的距离为40px
