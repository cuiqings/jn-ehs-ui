<template>
  ...
  <!-- 判卷后：自定义渲染，不用 ant radio，避免蓝色圆圈残留 -->
  <div v-else>
    <div
      ...
      :key="v.key"
      :class="{
        'option-correct': currentQuestion.userAnswer === v.key && currentQuestion.correctFlag,
        'option-wrong':   currentQuestion.userAnswer === v.key && !currentQuestion.correctFlag,
      }"
    >
      <span class="option-icon">
        <CheckCircleFilled v-if="currentQuestion.userAnswer === v.key && currentQuestion.correctFlag" class="icon-correct" />
        <CloseCircleFilled v-else-if="currentQuestion.userAnswer === v.key && !currentQuestion.correctFlag" class="icon-wrong" />
