<template>
  <div>
    ...
    <BasicTable @register="registerTable" :rowSelection="rowSelection">
      <!--插槽:table标题-->
      <template #tableTitle>
        <a-button v-if="hasPermission('notice:ftz')" type="primary" @click="handleAdd" preIcon="ant-design:plus-outlined"> 发通知
        </a-button>
        <a-dropdown v-if="hasPermission('notice:del')&&selectedRowKeys.length > 0">
          <template #overlay>
            <a-menu>
    ...
    <!-- 表单区域 -->
    <EhsNoticeInfoModal @register="registerDrawer" @success="reload" :showFooter="showFooter"></EhsNoticeInfoModal>
    <EhsNoticeInfoDetail @register="registerDesc" @success="reload" :showFooter="showFooter"></EhsNoticeInfoDetail>
    <EhsNoticeInfoReviewing @register="registerReviewing" @success="reload" :showFooter="showFooter"></EhsNoticeInfoReviewing>
          ...
          <tr v-if="type1||type2||type3||type4" style="height:160px;  border: 1px solid #000;">
            <td colspan="7" class="STYLE3" style="text-align: left; border:1px solid #000;">
              ...
              <div style="width: 30px;height: 30px;margin-left: 15px">
                ...
                <a-modal :visible="previewVisible" :footer="null" @cancel="imgCancel(signatureImg)">
                  <img alt="example" style="width: 100%" :src="previewImage" />
                </a-modal>
              </div>
            ...
            <td colspan="7" class="STYLE3" style="text-align: left; border:1px solid #000;">
              ...
              <div style="width: 30px;height: 30px;margin-left: 15px">
                ...
                <a-modal :visible="previewVisible" :footer="null" @cancel="imgCancel(recipientSignImg)">
                  <img alt="example" style="width: 100%" :src="previewImage" />
                </a-modal>
              </div>
              <div style="float: right;margin-top: 70px;margin-right: 10px">
            ...
            <td colspan="12" class="STYLE3">
              ...
              <div style="width:240px;margin-bottom: 10px;margin-right: 20px">
                <div style="float: right;margin-right: -295px;">
                  ...
                  <a-modal :visible="previewVisible" :footer="null" @cancel="imgCancel(correctorSignImg)">
                    <img alt="example" style="width: 100%" :src="previewImage" />
                  </a-modal>
                </div>
            ...
            <td colspan="12" class="STYLE3">
              ...
              <div style="width:240px;margin-bottom: 10px;margin-right: 20px">
                <div style="float: right;margin-right: -295px;">
                  ...
                  <a-modal :visible="previewVisible" :footer="null" @cancel="imgCancel(reviewSignImg)">
                    <img alt="example" style="width: 100%" :src="previewImage" />
                  </a-modal>
                </div>
                <div style="float: right;margin-right: -291px;margin-top: 137px;margin-bottom: 10px;">
...
import EhsNoticeInfoReviewing from './components/EhsNoticeInfoReviewing.vue'
...
import {deleteRecord} from "/@/views/configManage/cleanProject/plan.api";
...
const [registerReviewing, { openDrawer: openReviewing }] = useDrawer();
 ...
 * 审核
 */
function handleReviewing(record: Recordable) {
  showFooter.value = true;
  openReviewing(true, {
    record,
    showFooter: showFooter.value
...
function handleDelete(record) {
  // if(record.status=='0'||record.status=='1'||record.status=='11'){
    createConfirm({
      iconType: 'warning',
      title: '删除',
      content: '确定要删除吗？',
  ...
  // }else{
  //   createInfoModal({ title: '提示', content: '该状态不能删除！' });
  // }
}
...
async function batchHandleDelete() {
  ...
  //   if(!(selectedRows.value[i].status=="0"||selectedRows.value[i].status=="1"||selectedRows.value[i].status=="11")){
  //     createInfoModal({ title: '提示', content: '已下发状态数据不可删除！' });
  //     return;
  //   }
  // }
...
function getTableAction(record) {
  if(record.auditorId == userinfo.value.id&&record.status =='1'){
    return [
      {
        label: '编辑',
        onClick: handleEdit.bind(null, record),
        ifShow: () => {
          return (record.status === '0' || record.status === '11') && hasPermission('notice:edit');
        },
      },
      {
        label: '查看',
        ...
        ifShow: () => {
          return hasPermission('notice:detail');
        },
      },
      {
        label: '删除',
        ...
        ifShow: () => {
          return hasPermission('notice:del');
        },
      },
      {
        label: '审核',
        onClick: handleReviewing.bind(null, record),
        ifShow: () => {
          return record.status === '1' && hasPermission('notice:reviewing');
        },
      }
    ]
  }else{
...
function getDropDownAction(record) {
  if(record.auditorId == loginInfo.value.userInfo.id&&record.status =='1'){
    return [
      {
        label: '查看',
        onClick: handleDetail.bind(null, record),
...
async function getAttachment(record: Recordable) {
  ...
  // }, {isTransformResponse: false});
  // if (res.success && res.result) {
  //   window.open(res.result+"?url="+encodeURIComponent(Base64.encode(picPreviewUrl+record.attachment)), '_blank');
  // } else {
  //   console.log('数根节点查询结果异常', res);
  // }
}
...
function showViewModal(record: Recordable) {
  ...
  console.log(ehsNoticeInfoForm.ehsNoticeInfo.title);
