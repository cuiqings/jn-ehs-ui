    ...
    <van-form ref="formEl">
      <van-row justify="space-between" style="margin-top: 20px">
        <div class="" style="margin-left: 10px">
          <span style="color: red">*</span><span>复查意见：</span>
        </div>
      </van-row>
      <van-row justify="space-between" style="margin-top: 20px">
        <van-cell inset>
      ...
      <div class="buttonClass1" type="primary" native-type="submit" align="center" @click="handleOk">
        <span>
          复查通过
        </span>
      </div>
      <div style="background-color: #F59A23" type="primary" native-type="submit" class="buttonClass1" align="center" @click="handleNok">
        <span>
          不通过
        </span>
      </div>
...
//不通过
async function handleNok() {
  formEl.value?.validate().then(async() => {
    console.log("=====app-不通过=====",imgList);
    //获取通知书类型字典
    let dicurl = '/sys/dict/getDictText/'+"notice_info_type/"+ item.type;
    let infoTypeText = '';
    await defHttp.get({url: dicurl,params: {}}).then(res=>{
      infoTypeText = res;
    });
  let values = {
    id: item.correctionList[item.correctionList.length-1].id,
    noticeInfoId: item.id,
    reviewImg: imgList.value,
    reviewOpinion: reviewOpinion.value,
    auditUserId : userinfo.value.id,
    auditName : userinfo.value.realname,
    status: '2',
    infoTypeText: infoTypeText,
  };
  ...
  let infoFormData = {'id': item.id,'status': '3',reviewTime: dayjs(),reviewSign: userinfo.value.signature};
...
async function handleOk() {
  formEl.value?.validate().then(async() => {
    ...
    let infoFormData = {
      ...
      'status': '5',
      reviewTime: dayjs(),
      reviewSign: userinfo.value.signature
    };
    await defHttp.post({url: infourl, params: infoFormData}, {successMessageMode: 'none'});
    router.push({
      path: `/app/ehsNoticeInfoRecDetail`,
