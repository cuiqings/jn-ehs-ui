...
export const postFileList = (params?) => {
  return defHttp.post({ url: `/hb/filemanage/api/getFileList?token=${getToken()}`, params });
};
export const add = (params?) => {
  return defHttp.post({ url: '/hb/filemanage/api/create', params });
};
// 上传
export const uploadFile = (params, fileId: string | number) => {
  return defHttp.post({ url: `/hb/filemanage/api/uploadFile?token=${getToken()}&target=${fileId}`, params });
};
// 下载
...
export const showFileLog = (id: string, page: number, limit: number) => {
  return defHttp.get({ url: `/hb/filemanage/api/showFileLog?id=${id}&page=${page}&limit=${limit}&column=updateTime&order=desc` });
};
// 获取用户
...
export const doAuth = (params) => {
  return defHttp.post({ url: '/hb/filemanage/api/doAuth', params });
};
// 获取当前权限
export const getInitAuth = (id: string) => {
  return defHttp.get({ url: `/hb/filemanage/api/getInitAuth?token=${getToken()}&id=${id}` });
};
// 获取角色信息
