<template>
  <div class="risk-identify">
      ...
      <div>
        ...
        <div class="flex-align_center">
          <div class="flex-align_center border-right">
            <div>
              <div style="color: rgb(200, 0, 22)">{{ evaluationResultsCount.dps }}</div>
              <div style="color: rgb(200, 0, 22)">待评审</div>
            </div>
            <div>
              <div style="color: rgb(200, 0, 22)">{{ evaluationResultsCount.psz }}</div>
              <div style="color: rgb(200, 0, 22)">评审中</div>
            </div>
            <div>
              <div style="color: rgb(200, 0, 22)">{{ evaluationResultsCount.dsh }}</div>
              <div style="color: rgb(200, 0, 22)">待审核</div>
            </div>
            <div>
              <div style="color: rgb(200, 0, 22)">{{ evaluationResultsCount.shz }}</div>
              <div style="color: rgb(200, 0, 22)">审核中</div>
            </div>
          </div>
          <div class="flex-align_center">
            <div>
    ...
    <basic-table @register="registerTable" :rowSelection="rowSelection" ref="tableRef">
      <template #tableTitle>
        <a-button type="primary" v-auth="'processChecklist:review'" v-if="selectedRowKeys.length > 0" @click="handleReview(selectedRowKeys)"
          >专业评审</a-button
        >
        <a-button
          type="primary"
          v-auth="'processChecklist:submitReview'"
          v-if="selectedRowKeys.length > 0"
          @click="handleSubmitReview(selectedRowKeys)"
          >提交审核</a-button
        >
        <a-button preIcon="ant-design:export-outlined" type="primary" v-auth="'processChecklist:export'" @click="onExportXls"> 导出</a-button>
      </template>
  ...
  <controlModal @register="registerControlModal" @success="handleSuccess" />
  <dangerSourceModal @register="registerDangerSourceModal" @success="handleSuccess" />
</template>
<script setup name="risk-processChecklist" lang="ts">
  import { BasicTable, TableAction } from '/@/components/Table';
  import { useMainContent } from './hooks/useMainContent';
  import reviewModal from './reviewModal.vue';
  import controlModal from './controlModal.vue';
  import dangerSourceModal from './dangerSourceModal.vue';
  const {
    registerTable,
    getActions,
    rowSelection,
