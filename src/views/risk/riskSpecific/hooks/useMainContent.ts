...
export const useMainContent = (tableRef) => {
  ...
  const { tableContext, onExportXls, operateRequest } = useListPage({
    designScope: 'risk-Specific-list',
    tableProps: {
      api: list,
      columns: columns,
      formConfig: {
  ...
  const getActions = (record: any) => {
    return [
      // {
      //   label: '编辑',
      //   auth: ['riskSpecific:edit'],
      //   onClick: handleEdit.bind(null, record),
      // },
      {
        label: '删除',
        auth: ['riskSpecific:del'],
