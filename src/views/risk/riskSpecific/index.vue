<template>
  <div class="risk-identify">
    ...
    <basic-table @register="registerTable" :rowSelection="rowSelection" ref="tableRef">
      <template #tableTitle>
        <a-button preIcon="ant-design:plus-outlined" type="primary" v-auth="'riskSpecific:add'" @click="handleAdd">新增</a-button>
        <a-button preIcon="ant-design:export-outlined" type="primary" v-auth="'riskSpecific:export'" @click="onExportXls"> 导出</a-button>
        <a-button preIcon="ant-design:delete-outlined" v-auth="'riskSpecific:batchDel'" v-if="selectedRowKeys.length > 0" @click="batchHandleDelete">
          批量删除</a-button
        >
      </template>
  ...
  <risk-specific-drawer @register="registerDrawer" @success="handleSuccess" />
</template>
<script setup name="risk-riskSpecific" lang="ts">
  import { BasicTable, TableAction } from '/@/components/Table';
  import { useMainContent } from './hooks/useMainContent';
  import { ref } from 'vue';
  import riskSpecificDrawer from './drawer.vue';
  const tableRef = ref();
  const {
    registerTable,
    getActions,
