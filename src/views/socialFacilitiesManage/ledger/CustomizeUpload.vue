  ...
  <div :class="isType === 'pc' ? 'material-upload' : ''">
    ...
    <div class="material-upload-div">
      <div
        ...
        :class="!disabled ? 'material-upload-div_block' : 'material-upload-div_none'"
        :key="index"
      ...
      >
        ...
        <DeleteOutlined class="material-upload-div_del" v-show="item.show && !disabled" @click="onDel(index)" />
        <a-button type="link" v-if="disabled" @click="downloadFile(item.downloadUrl)">下载</a-button>
      </div>
      ...
      <a-upload
        ...
        @change="onFileChange"
      >
        <div class="file-btn" v-if="fileList.length !== maxCount && !disabled && loading !== 'uploading'">添加附件</div>
      </a-upload>
    </div>
  </div>
</template>
<script setup name="replenish-material" lang="ts">
  ...
  const props = defineProps({
    ...
    bizPath: {
      type: String,
      required: false,
      default: 'socialFacilitiesManage',
    },
  });
  ...
  const onPreview = (file) => {
    previewFile(file.downloadUrl).then((res) => {
  ...
  const onFileChange = (info) => {
    ...
    fileList1.value = arr.map((item) => {
      return {
        ...
        downloadUrl: item.response.message,
        show: false,
        response: {
          success: true,
