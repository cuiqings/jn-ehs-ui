<template>
  <BasicDrawer forceRender showFooter v-bind="$attrs" @register="register" @close="close" :title="title" width="65%" @ok="handleSubmit"
    destroyOnClose>
    <div style="width: 100%;">
...
import { evaluateAdd, evaluateEdit } from '../api'
...
const handleSubmit = () => {
  formRef.value?.validate().then(() => {
    ...
    if (title.value == '新增') {
      evaluateAdd(formState).then(res => {
      ...
      evaluateEdit(formState).then(res => {
        changeOkLoading(false);
        emits('submitSuccess', res);
        closeDrawer();
        formRef.value?.resetFields();
