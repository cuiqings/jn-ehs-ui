        ...
        <a-row :gutter="12" v-if="['上传'].includes(title)">
          ...
          <a-col :span="12">
            ...
            >
              ...
              <a-select disabled v-model:value="formState.type" placeholder="请选择">
...
<script lang="ts" setup>
  ...
  import { physicalExaminationEdit, physicalExaminationUpd, physicalExaminationInfo, physicalExaminationSave, physicalExaminationDetail, deleteReport } from '../api';
    ...
    if (title.value === '上传') {
      ...
      physicalExaminationInfo({ id: data.record.id }).then((res) => {
        if(res.infoList && res.infoList.length > 0) {
          Object.assign(formState, res.infoList[0]);
        } else {
          delete formState.id;
        }
      });
  ...
  const handleSave = () => {
    ...
    physicalExaminationSave(formState)
    .then((res) => {
      closeDrawer();
    ...
    formRef.value
      ...
      .then(() => {
        ...
        if(title.value === '上传') {
          openSignModal(true, {
            callback: (sign) => {
              physicalExaminationEdit({ ...formState, ...sign })
                .then((res) => {
                  changeOkLoading(false);
          ...
          physicalExaminationUpd(formState)
            .then((res) => {
              changeOkLoading(false);
              emits('submitSuccess', res);
              closeDrawer();
