<template>
  <BasicTable @register="registerTable" :rowSelection="rowSelection">
    ...
    <template #attachment="{ record }">
      <span style="cursor: pointer; color: #1890ff" @click="downloadFile(record.attachment)">{{ formatFileName(record.attachment) }}</span>
    </template>
  ...
  <!--编辑抽屉-->
  <AccidentDangerModal @register="registerModal" @success="reload" />
</template>
<script lang="ts" setup>
  import { ref } from 'vue';
  import { BasicTable, TableAction } from '/@/components/Table';
  import { useModal } from '/@/components/Modal';
  import AccidentDangerModal from './AccidentDangerModal.vue';
  import { columns, searchFormSchema } from './accidentDanger.data';
  import { list, batchDelete, deleteById, getExportUrl, getImportUrl } from './accidentDanger.api';
