<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="选择角色/人" @ok="onSubmit" @cancel="onCancle" width="900px" destroyOnClose>
    <a-tabs v-model:activeKey="activeKey" @change="tabChange">
<!--        <a-tab-pane key="1" tab="按角色"></a-tab-pane>-->
        ...
        <div class="role-box" v-if="activeKey==='1'">
          ...
          <div class="ur-ovl">
            <a-checkbox-group v-model:value="roleValues" style="width: 100%" @change="roleChange">
              <a-row>
                <a-col :span="24" v-for="(item) in roleList">
...
function getDeptList(val?: string) {
  ...
  defHttp.get({ url: '/sys/sysDepart/queryTreeListCompliance', params }).then((res) => {
    gData.value = res != null ? res : [];
    expandedKeys.value= res != null ? res.map(v => v.key) : [];
  });
}
...
function getUserList(val?: string) {
  ...
  defHttp.get({ url: '/jn/common/getUser', params }).then((res) => {
...
async function onSubmit() {
  ...
  const params = {
    ...
    approvalContentOne: selectValuesData.map(v => v.value).join(','),
    approvalContentTwo: selectValuesData.map(v => v.name).join(','),
    approvalContentThree: selectValuesData.map(v => v.orgCode).join(','),
  };
...
async function onCancle(){
  const selectValuesData = [...defaultValues.value];
  const params = {
    field: field.value,
    approvalType: activeKey.value === '2' ? '1' : '0',
    approvalContentOne: selectValuesData.map(v => v.value).join(','),
...
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
  ...
  data.auditIds != "" ? data.auditIds.split(",") : [];
  if(data.auditIds != "") {
    await defHttp.get({url: '/sys/user/getUserByDepartId', params: {"userIds": data.auditIds}}).then((res) => {
      selectValues.value = res.map((item: any) => {
        return {
          name: item.realname,
          value: item.id,
          orgCode: item.orgCode,
      ...
      defaultValues.value = [...selectValues.value];
    });
