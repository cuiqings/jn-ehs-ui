<template>
  <div>
    <BasicTable @register="registerTable" :rowSelection="rowSelection" :searchInfo="searchInfo">
      ...
      <template #signSheetPath="{ record }">
        <a @click="handlePreview(record.signSheetPath)">{{ record.signSheetPath?.split('/').slice(-1)[0] }}</a>
      </template>
      <template #recordSheetPath="{ record }">
        <a @click="handlePreview(record.recordSheetPath)">{{ record.recordSheetPath?.split('/').slice(-1)[0] }}</a>
      </template>
      <template #resultPath="{ record }">
        <a @click="handlePreview(record.resultPath)">{{ record.resultPath?.split('/').slice(-1)[0] }}</a>
      </template>
      <template #recordPath="{ record }">
        <a @click="handlePreview(record.recordPath)">{{ record.recordPath?.split('/').slice(-1)[0] }}</a>
      </template>
    ...
    <TrainModal @register="registerModal" @success="reload" />
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import { getList, deleteRecord, batchDeleteRecord, getOrganizationNew } from './train.api';
  import { columns, searchFormSchema } from './train.data';
  ...
  const { tableContext, doRequest } = useListPage({
    ...
    tableProps: {
      ...
      beforeFetch: (info) => {
        const { trainTime } = info;
        if (trainTime) {
          const array = trainTime.split(',');
