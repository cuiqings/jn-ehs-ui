<template>
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" title="巡检记录查看" width="1400px" destroyOnClose>
    <!-- 计划信息 -->
    <div class="info-wrapper">
      ...
      <div class="info-content">
        ...
        <div style="display: flex"
          ><div class="item">
            <div class="item-title">计划类型：</div>
            <div class="item-content" v-if="planData.planType == '1'">长期计划</div>
            <div class="item-content" v-else>临时计划</div>
          </div>
          <div class="item">
            <div class="item-title">计划名称：</div>
            <div class="item-content">{{ planData.planName }}</div>
          </div></div
        >
        <div style="display: flex"
          ><div class="item">
            <div class="item-title">巡检周期：</div>
            <div class="item-content">{{ filterDictTextByCache('yh_inspection_cycle', planData.checkCycle) }}</div>
          </div>
          <div class="item">
            <div class="item-title">执行日：</div>
            <div class="item-content">{{ planData.executeDate }}</div>
          </div></div
        >
        <div style="display: flex">
          <div class="item">
            <div class="item-title">执行角色：</div>
            <div class="item-content">{{ planData.executeRoleName }}</div>
          </div>
          <div class="item">
            <div class="item-title">监督人：</div>
            <div class="item-content">{{ planData.superviseRoleName }}</div>
          </div></div
        >
        <div style="display: flex">
          <div class="item">
            <div class="item-title">创建时间：</div>
            <div class="item-content" v-if="historyData.executeDate">{{ historyData.executeDate.slice(0, 10) }}</div>
          </div></div
        >
      </div>
    </div>
    ...
    <div class="history-wrapper">
      <div class="info">
        <div class="split-line"></div>
        <div class="info-title">巡检记录</div>
      </div>
      <div class="info-content">
        <div style="display: flex">
          <div class="item">
            <div class="item-title">执行人：</div>
            <div class="item-content">{{ historyData.executePeopleName }}</div>
          </div>
          <div class="item">
            <div class="item-title">执行时间：</div>
            <div class="item-content">{{ historyData.realExecuteDate }}</div>
          </div>
        </div>
      </div>
      <div>
