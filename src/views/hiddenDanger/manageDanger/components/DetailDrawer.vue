  ...
  <BasicDrawer v-bind="$attrs" @register="registerDrawer" title="查看" width="1000px" destroyOnClose>
    <div>
      <a-tabs v-model:activeKey="activeKey" type="card">
                  ...
                  <div class="his-content">
                    ...
                    <div class="history-explain">
                      ...
                      <div class="explain-text">
                        ...
                        <span style="display: inline-block; width: 650px" v-else>{{ item.description }}</span>
                      </div>
                    </div>
        ...
        <a-tab-pane key="4" v-if="accountPage == '4'" tab="整改通知">
          <div v-if="noticeNos.length > 0">
            <NoticeSingle v-for="(item, index) in noticeNos" :key="index" :noticeNo="item" />
          </div>
        </a-tab-pane>
      </a-tabs>
    </div>
...
<script lang="ts" setup>
  ...
  import { BasicDrawer, useDrawerInner, useDrawer } from '/src/components/Drawer';
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    noticeNos.value = [];
    // 隐患台账
    if (data.accountPage && data.accountPage == '4') {
      accountPage.value = '4';
      if (data.record.noticeNos) {
        noticeNos.value = data.record.noticeNos.split(',');
      }
    }
    ...
    if (data.record.sourceType == '1') {
      ...
      if (res.success && res.result) {
        ...
        if (res.result.checkPicture) {
          let picList = res.result.checkPicture.split(',');
          picList.map((item) => {
            fileListBak.push({
              uid: item,
              name: item,
