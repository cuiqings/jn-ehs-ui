<template>
  <div>
    <BasicModal v-bind="$attrs" @register="registerModal" title="选择处理人" @ok="onSubmit" width="900px">
          ...
          <div class="user-box">
            <a-row>
              <a-col :span="12" style="border-right: 1px solid #eee; padding-right: 10px; min-height: 200px">
                <a-input-search placeholder="请输入" v-model:value="searchValue" style="margin-bottom: 10px" @search="onUserSearch" allowClear />
                <div class="ur-ovl">
                  <a-tree
                    v-if="gData.length > 0"
                    class="dep-tree"
                ...
                <a-spin :spinning="spinning">
                  ...
                  <div class="ur-ovl">
                    <!-- <a-checkbox-group v-model:value="state.checkedList" style="width: 100%">
                      <a-row>
                        <a-col :span="24" v-for="item in plainOptions" :key="item.id">
                          <a-checkbox :value="item.value" @change="userCheckChange">{{ item.label }}</a-checkbox>
                        </a-col>
                      </a-row>
                    </a-checkbox-group> -->
                    <a-radio-group v-model:value="state.checkedList" @change="userCheckChange" :options="plainOptions">
                      <!-- <a-row>
...
async function loadRoot() {
  ...
  if (res.success && res.result) {
    ...
    expandedKeys.value = res.result.map((v) => v.key);
  } else {
...
function getUserList(val?: string) {
  ...
  defHttp
    .get({ url: '/jn/common/getAllUserByOrg', params })
    .then((res) => {
      if (!res) {
        return;
      }
...
function onUserSearch(value: string) {
  if (value) {
    ...
    let params = {
      realname: value,
      orgCode: gData.value[0].orgCode,
    };
    defHttp
      .get({ url: '/jn/common/getUserList', params })
      .then((res) => {
        plainOptions.value = res.map((item: any) => {
          return {
            label: item.realname,
...
async function onSubmit() {
  ...
  const params = {
    // field: field.value,
    newUserName: selectValuesData.map((v) => v.value).join(','),
...
const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
  ...
  historyIds.value = data.historyIds;
  // 下次点击弹窗的时候清空表单
  // await resetFields();
  activeKey.value = '1';
  plainOptions.value = [];
