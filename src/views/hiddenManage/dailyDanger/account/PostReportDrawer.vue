<template>
  ...
  >
    <BasicForm @register="registerForm">
      <template #teamLeader="{ model, field }">
        <JUserModal v-model:value="model[field]" placeholder="请选择" @confirm="confirmTeamLeader" />
      </template>
    </BasicForm>
...
<script lang="ts" setup>
  ...
  import { dangerReport, getTeamLeaderList } from './account.api';
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    fileList.value = '';
    fileList1.value = '';
    // 重置表单 - 使用 nextTick 确保表单已注册
    await nextTick();
    try {
      await resetFields();
    } catch (error) {
      console.warn('表单重置失败，可能表单尚未完全初始化');
    }
  });
  // 注册表单
  ...
  const getTeamLeader = async () => {
    await getTeamLeaderList().then((res) => {
      console.log('getTeamLeaderList res', res);
      if (res.length > 0) {
        // teamLeader
        setFieldsValue({
          teamLeader: res[0].id,
  ...
  function confirmTeamLeader(userName: string, userValue: string, orgCode: string) {
    clearValidate(['teamLeader']);
  }
</script>
