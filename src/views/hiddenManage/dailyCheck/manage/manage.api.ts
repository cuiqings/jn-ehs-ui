...
enum Api {
  ...
  rgLcList = '/rk/skRkRecog/getList',
  startOrStopBatch = '/yh/skYhPlan/updateUseFlagBatch',
}
...
export const startBatch = (params, handleSuccess) => {
  return defHttp.post({ url: Api.startOrStopBatch, params }).then(() => {
    handleSuccess();
  });
}
/**
 ...
 * @param params
 */
export const stopBatch = (params, handleSuccess) => {
  return defHttp.post({ url: Api.startOrStopBatch, params }).then(() => {
    handleSuccess();
  });
}
