<template>
  <div>
    <basic-modal v-bind="$attrs" @register="registerModal" :width="size[workType].w" :title="title" wrap-class-name="mark-modal" destroyOnClose @cancel="close">
...
<script lang="ts" name="grid-modal" setup>
  ...
  const title = ref('请在图中点击，选择作业施工位置（双击取消选择）');
  ...
  const [registerModal, { closeModal }] = useModalInner(async (data) => {
    ...
    baseMap.value = data.baseMap ? await getFileAccessHttpUrl(data.baseMap) : management;
    nextTick(() => {
