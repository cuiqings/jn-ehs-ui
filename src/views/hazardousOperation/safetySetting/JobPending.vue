<template>
  <div>
          ...
          <a-col :span="6" v-for="item in cardData" :key="item.id">
            <a-card class="fixed-height-card">
                  ...
                  <div style="display: flex; justify-content: space-between">
                    <div>
                      <span style="font-weight: bold; display: inline-block">
                        {{ item.workTypeName || '非高危作业' }}
                      </span>
                    </div>
                    <div>
                      <a-checkbox :value="item.workType"></a-checkbox>
                    </div>
                  </div>
                </a-col>
                <a-col :span="24" style="margin-top: 10px">安全措施：</a-col>
                <div class="text-main">
              ...
              <a-row :span="24" v-if="state == '1'" justify="end" :gutter="[20, 10]" style="margin-top: 15px">
                <a-button type="text" danger @click="del(item.workType)">删除</a-button>
    ...
    >
      ...
      >
        <a-form-item label="作业类型" name="workType" :colon="false" required>
          <a-select :disabled="['编辑', '查看'].includes(title)" v-model:value="addAnalysisForm.workType" allowClear placeholder="请选择">
...
<script setup lang="ts">
  ...
  import type { CardList, searchJobList } from '../type';
  ...
  const del = (type, msg) => {
    Modal.confirm({
      ...
      content: msg || '是否确认删除？',
      onOk: async () => {
        await securityDel({ workType: type });
        getList({});
        workTypes.value = [];
      },
    });
  ...
  const edit = (item) => {
    addAnalysisForm.workType = item.workType;
  ...
  const getList = async (data: searchJobList) => {
    ...
    res.forEach((item) => {
      item.workTypeName = work_type.value.find((i) => i.value === item.workType)?.label;
    });
    cardData.value = res;
  };
