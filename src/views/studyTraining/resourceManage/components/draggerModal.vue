  ...
  const beforeUpload = async (file: any) => {
    ...
    if (props.fileType === 'AUDIO') {
      const isAllowedType =
        ...
        file.type === 'audio/x-ms-wma'; // wma
      if (!isAllowedType) {
        return message.error('上传格式错误，请上传.mp3、.aac、.wav、.wma格式的音频！');
      }
    }
    if (props.fileType === 'ANNEX') {
      const extension = file.name.split('.').pop().toLowerCase();
      const isAllowedType =
        file.type === 'application/zip' || // .zip
        file.type === 'application/x-zip-compressed' || // .zip (alternative)
        file.type === 'application/octet-stream' || // .zip (generic binary)
        extension === 'rar'; // .rar
      if (!isAllowedType) {
        return message.error('上传格式错误，请上传.rar、.zip格式的附件！');
      }
    }
  ...
  const upload = async (file) => {
    ...
    if (props.fileType === 'AUDIO') {
      ...
      audioElement.addEventListener('loadedmetadata', async () => {
        const duration = parseInt(audioElement.duration); //时长为秒，取整
